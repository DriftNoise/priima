...
def make_arctic_grid(filepath: str):
    """
    Create a NetCDF file as used in TOPAZ4. The grid is used to resample
    data to it and is a replacement for copy+pasting an old TOPAZ4 data file
    just to use its grid. Instead, the grid is shaped here in the exact same
    way, just with much less disk space needed. It is required to transform
    the input data to be able to calculate ice drift trajectories in a
    ...
    metno_proj4_string = (
        "+proj=stere +lat_0=90 +lat_ts=90 +lon_0=-45 +k=1 +datum=WGS84 "
        "+units=m +no_defs"
    )
    outProj = Proj(metno_proj4_string)
    ...
    crso.longitude_of_projection_origin = -45.
...
def make_antarctic_grid(filepath: str):
    """
    Create a NetCDF file in EPSG:3031 which can be used to resample
    meteorological data to. It is required to transform
    the input data to be able to calculate ice drift trajectories in a
    cartesian coordinate system.
    """
    xcells = 780
